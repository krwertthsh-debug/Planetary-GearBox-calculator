...
def generate_openscad_assembly(zs, zp, zr, m, n_planets, d_pin, b, 
                               ...
                               din, dout, pin_span):
    """Generate OpenSCAD code for the full planetary gearbox"""
...
// Generated by Planetary Gearbox Designer v3
// =====================================================
...
// === SIMPLIFIED GEAR MODULE (Trapezoidal tooth) ===
// NOTE: For true involute profiles, use the 'gears.scad' library from OpenSCAD
module gear_approx(z, m, width, tip_d, root_d) {{
    ...
    // Add teeth (trapezoidal approximation)
    for (i = [0 : z-1]) {{
        rotate([0, 0, i * tooth_angle]) {{
            translate([pitch_r, 0, 0]) {{
                // Trapezoid tooth using polyhedron
                polyhedron(
                    points = [
...
module ring_gear() {{
    difference() {{
        ...
        // Internal teeth (simplified as rectangular blocks)
        for (i = [0 : {zr - 1}]) {{
            rotate([0, 0, i * 360 / {zr}]) {{
...
module assembly() {{
    ...
    // Input shaft (below sun)
    translate([0, 0, -{in_shaft_len/2 + b/2:.2f}])
        input_shaft();
    
    // Output shaft (above carrier)
...
// Render full assembly (uncomment to show):
assembly();
"""
    return code
...
st.markdown('<div class="sub-header">Complete Single-Stage 1:9 Reduction — All Component Calculations + 3D Visualization + OpenSCAD</div>', unsafe_allow_html=True)
